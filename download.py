...
def create_video_folders(dataset, output_dir, tmp_dir):
    ...
    label_to_dir = {}
    for label_name in dataset['label'].unique():
...
def construct_video_filename(row, label_to_dir, trim_format='%06d'):
    ...
    """
    basename = '%s.mp4' % (row['videoname'])
    if not isinstance(label_to_dir, dict):
        dirname = label_to_dir
    else:
        dirname = label_to_dir[row['label']]
...
def download_clip_wrapper(row, label_to_dir, trim_format, tmp_dir):
    ...
    downloaded, log = download_clip(row['video_id'], output_filename,
                                    tmp_dir=tmp_dir)
...
def main(input_csv, output_dir,
         trim_format='%06d', num_jobs=-1, tmp_dir='/tmp/kinetics',
    ...
    #with open('download_report.txt', 'w') as fobj:
    #    fobj.write('\n'.join('%s' % str(id) for id in status_lst))


if __name__ == '__main__':
    description = 'Helper script for downloading and trimming kinetics videos.'
    p = argparse.ArgumentParser(description=description)
    p.add_argument('input_csv', type=str, default='test.csv',
